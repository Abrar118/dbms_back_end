...
create or replace view admin_view as
    select ADMIN.ADMIN_ID, name, email, gender, house || ', ' || street || ', ' || city as Address, to_char(dob, 'dd-mm-yyyy') as "Date of Birth", floor(months_between(sysdate, dob)/12) as Age,
          designation ,listagg(PHONE_NO, ', ') as Phone
    from ADMIN JOIN ADMIN_PHONE on ADMIN.ADMIN_ID=ADMIN_PHONE.ADMIN_ID
    group by ADMIN.ADMIN_ID, name, email, gender, city, street, house, dob, designation
...
create or replace view staff_view as
    select staff.staff_ID, name, email, gender, house || ', ' || street || ', ' || city as Address, to_char(dob, 'dd-mm-yyyy') as "Date of Birth", floor(months_between(sysdate, dob)/12) as Age,salary,
           listagg(PHONE_NO, ', ') as Phone
    from staff JOIN staff_PHONE on STAFF.STAFF_ID=STAFF_PHONE.STAFF_ID
    group by STAFF.STAFF_ID, name, email, gender, city, street, house, dob,salary
...
select CUSTOMER.CUSTOMER_ID, name, email, gender, house || ', ' || street || ', ' || city as Address, to_char(dob, 'dd-mm-yyyy') as "Date of Birth", floor(months_between(sysdate, dob)/12) as Age,
       listagg(PHONE_NO, ', ') as Phone
from CUSTOMER JOIN CUSTOMER_PHONE on CUSTOMER.CUSTOMER_ID=CUSTOMER_PHONE.CUSTOMER_ID
group by CUSTOMER.CUSTOMER_ID, name, email, gender, city, street, house, dob
...
from (
    ...
    from
        (select VETERINARIAN.vet_id, name, email, gender, house || ', ' || street || ', ' || city as Address, dob, salary, qualification, count(rescued_animal_id) as checkup_count
        from VETERINARIAN, CHECKUP_RESCUE
        where VETERINARIAN.vet_id=CHECKUP_RESCUE.vet_id(+)
        group by name, VETERINARIAN.vet_id, email, gender, house || ', ' || street || ', ' || city , dob, salary, qualification
        ) A
    ,
...
begin
    ...
    if temp_id like 'd%' then
        ...
        if temp_cabin_type!='isolated' then
            ...
            for i in (select * from CABIN where lower(type)='isolated')
            loop
                if i.capacity>0 and upper(i.animal_type)=upper(temp_animal_type) then
                    update DAYCARE_ANIMAL
                        set cabin_no=i.cabin_no
